...
WORKDIR /src
COPY ["ExerciciosAlura.csproj", "."]
RUN dotnet restore "./ExerciciosAlura.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./ExerciciosAlura.csproj" -c %BUILD_CONFIGURATION% -o /app/build

# Esta fase é usada para publicar o projeto de serviço a ser copiado para a fase final
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./ExerciciosAlura.csproj" -c %BUILD_CONFIGURATION% -o /app/publish /p:UseAppHost=true
...
ENTRYPOINT ["ExerciciosAlura.exe"]